...
export class EscrowService {
  ...
  async createEscrow(userId: string, data: any) {
    ...
    let seller = await this.userRepository.findOne({ where: { email: data.seller_email_or_wallet } });
    if (!seller) {
        seller = await this.userRepository.findOne({ where: { wallet_address: data.seller_email_or_wallet } });
    }
  ...
  async registerDeposit(escrowId: string, txHash: string, fromAddress: string) {
      ...
      if (!txInfo.exists) throw new BadRequestException('Transaction not found on chain');
      ...
      const tx = this.txRepository.create({
          ...
          status: txInfo.confirmed ? TransactionStatus.CONFIRMED : TransactionStatus.PENDING,
